...
|---------|----------|--------|-----|-------|
| MBA L100 | 67.55 +/- 5.62 | 19.1 +/- 8.8 | -48pp | Possible data mismatch (TranAD vs SVDB) |
| SMD L100 | 52.07 | TBD | - | Still running (1hr+) |
| Exathlon | 18.64 | Not run | - | Planned |
| WADI | 64.91 | Not run | - | Planned |

**Working hypothesis for gap:** The MBA data from TranAD (identical train/test) is likely not the same as what the paper used. SVDB 4-record run (PID 179247) will clarify this.

**KEY FINDING:** Even at our lower F1=43.1% (single run), the raw AUROC=0.528 shows A2P's scoring is near-random. The F1 metric with tolerance window severely inflates apparent performance.
