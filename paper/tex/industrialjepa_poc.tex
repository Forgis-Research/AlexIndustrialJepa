...
\paragraph{Industrial Fault Detection.} Deep learning for fault detection has evolved from CNNs to transformers, but remains fundamentally \emph{single-machine} focused. The CWRU bearing dataset \cite{cwru} and C-MAPSS turbofan dataset \cite{cmapss} are ubiquitous benchmarks, yet both record sensor outcomes without commanded setpoints—preventing causal analysis of \emph{why} behavior changed. Recent surveys \cite{} identify cross-machine generalization as a critical open challenge.

\paragraph{JEPA for Time Series.} The Joint Embedding Predictive Architecture \cite{lecun2022path} learns by predicting masked embeddings rather than raw inputs. I-JEPA \cite{ijepa} demonstrated this for images; V-JEPA \cite{vjepa} extended it to video with multi-scale temporal masking.

Recent work adapts JEPA to time series: \textbf{TS-JEPA} \cite{tsjepa} targets communication-efficient remote control by encoding sensory data into compact embeddings; \textbf{MTS-JEPA} \cite{mtsjepa} introduces multi-resolution prediction with soft codebook bottleneck for anomaly prediction. Most relevant, Yan et al. \cite{yan2025industrial} apply JEPA to injection molding with action conditioning.

\textbf{Our key differentiation:} Prior work treats time series as observational—predicting $p(x_{t+1}|x_t)$. We exploit \emph{causal structure} of control systems, predicting $p(\text{effort}|\text{setpoint})$—the physics of how commands cause responses. This enables cross-machine transfer that observational models cannot achieve.

\paragraph{Transfer Learning in Robotics.} Cross-embodiment transfer has shown remarkable success in manipulation: Open X-Embodiment \cite{openx} demonstrates positive transfer across 22 robot types by training on diverse data. We investigate whether similar principles apply to fault detection—can a model trained on one robot detect faults on another by learning shared physics?

\paragraph{Causal Structure in Industrial Data.} FactoryNet \cite{factorynet} provides the first large-scale industrial dataset with explicit causal structure: every episode records Setpoint (command) $\rightarrow$ Effort (response) $\rightarrow$ Feedback (outcome). This mirrors the control-theoretic view where faults manifest as deviations in the command-response relationship, not in raw sensor statistics.
