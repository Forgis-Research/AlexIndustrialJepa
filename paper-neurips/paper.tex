...
% ============================================================
\begin{abstract}
Multivariate time series from industrial, infrastructure, and scientific systems contain early warning signals for \emph{grey swan events} - rare but partially predictable failures, anomalies, and threshold breaches that carry high stakes and few labels. We present a self-supervised method that predicts future sensor trajectories in latent space via a Joint Embedding Predictive Architecture (JEPA), yielding representations that support diverse grey swan tasks through trivial linear probes, and whose pretraining prediction error is itself a zero-label anomaly detector. On C-MAPSS FD001, a matched-architecture from-scratch ablation isolates the SSL contribution at $+8.8$ RMSE at full labels and $+16.9$ at 10\%; at 5\% labels the frozen encoder ($21.53 \pm 2.0$) surpasses matched-budget supervised STAR ($24.55 \pm 6.4$). An architectural ablation shows that \emph{causal} context is the key inductive bias: a bidirectional encoder with VICReg regularisation transfers uniformly worse across FD002-FD004 ($+6$ to $+11$ RMSE) and collapses at low labels. On SMAP spacecraft telemetry, the prediction-error anomaly score attains PA-F1 = 62.5\% with no anomaly labels (MTS-JEPA: 33.6\%); non-PA F1 sits near the $\approx7\%$ random baseline, so the detector is segment-localised rather than point-precise.
\end{abstract}
...
\begin{enumerate}[nosep]
  \item \textbf{Trajectory JEPA for grey swan prediction.} We adapt JEPA to multivariate time series via causal latent trajectory prediction, and demonstrate that a single pretrained encoder supports both time-to-failure regression (C-MAPSS) and zero-label anomaly onset detection (SMAP/MSL) through task-specific linear probes that share the same pretraining objective (\cref{sec:method,sec:benchmark,sec:anomaly_probe}).

  \item \textbf{Causal inductive bias matters more than model complexity.} A bidirectional context encoder with VICReg regularisation (V16b) attains comparable supervised fine-tuning RMSE but substantially worse frozen-probe generalisation (test RMSE 25.7 vs.\ 17.8), worse cross-machine transfer ($+6$ to $+11$ RMSE on FD002-FD004), and worse label efficiency at every budget (\cref{fig:cross_machine,fig:label_efficiency_v16b}, \cref{sec:ablations}).

  \item \textbf{Quantified pretraining contribution and label-efficiency crossover.} A matched-architecture from-scratch ablation isolates the SSL objective at $+8.8$ RMSE at full labels, growing to $+16.9$ at 10\% (\cref{sec:from_scratch}). At 5\% labels, the frozen SSL encoder ($21.53 \pm 2.0$) surpasses supervised STAR ($24.55 \pm 6.4$), demonstrating the practical payoff in the scarce-label regime (\cref{sec:label_efficiency}).

  \item \textbf{Zero-label anomaly detection as a byproduct of pretraining.} The pretraining prediction error itself serves as an anomaly score without any event labels, achieving PA-F1 62.5\% on SMAP (vs.\ 33.6\% for MTS-JEPA) and 43.3\% on MSL (\cref{sec:anomaly_probe}).
\end{enumerate}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_label_efficiency_v16b.pdf}
\caption{\textbf{Causal V2 vs.\ bidirectional V16b under varying FD001 label budgets.} E2E test RMSE; mean (line) and $\pm 1$ std ribbon over 3 seeds per point. V2 degrades gracefully from $14.2$ (100\%) to $25.3$ (5\%); V16b degrades sharply to $40.6$ at 5\% with $\sigma = 10.2$, a clear sign that the bidirectional encoder's gains do not transfer to scarce-label conditions.}
...
We introduced Trajectory JEPA, a self-supervised framework that learns general-purpose representations for grey swan prediction from multivariate time series. The central empirical finding is a label-efficiency crossover: the SSL objective contributes $+8.8$ to $+16.9$ RMSE over a matched from-scratch baseline, and at 5\% labels the frozen encoder beats supervised STAR; the anomaly probe (prediction error from pretraining) requires zero event labels.

A secondary architectural finding is that the \emph{causal} inductive bias dominates model complexity for grey-swan tasks: a bidirectional encoder with VICReg memorises domain-specific degradation profiles at the cost of frozen-probe generalisation, cross-machine transfer, and label efficiency.

Future work includes evaluation on real sensor data (addressing the simulation-dominated limitation), theoretical formalisation of the slow-feature learning mechanism, and extension to additional grey swan types (threshold exceedance, cascade failures). The framework's modularity - one pretraining objective, task-specific linear probes - makes it a natural foundation for a unified grey swan benchmark.
