...
\paragraph{SIGReg variant.} We also explore replacing EMA with SIGReg spectral regularization~\citep{balestriero2025lejepa}: a single shared encoder (no target copy) with a regularization term $\loss_\text{SIG}$ that prevents collapse by maintaining spectral diversity of the representation covariance. Results show SIGReg achieves competitive frozen probe quality (V15-SIGReg, 3 seeds: RMSE $= 9.2 \pm 1.5$, range 7.0--10.2 vs.\ V2-EMA: $17.8 \pm 1.7$) with $\sim$20\% fewer parameters (no predictor head). However, the probe trajectory exhibits high-frequency oscillations: re-running seed 42 gives best RMSE $= 11.6$ at epoch 70 vs.\ $10.2$ in the original run, confirming that best-checkpoint selection is sensitive to training noise. We use the EMA variant for all main results due to its training stability.
...
\begin{table}[t]
\centering
\caption{Architecture comparisons on C-MAPSS FD001. All use the same probe and fine-tuning protocol. V2 = main model (causal context, future-only target, EMA, L1 loss, 5 seeds). Full-seq target = variant where target sees full sequence $\x_{1:t+k}$ instead of future-only $\x_{t+1:t+k}$ (5 seeds, \cref{app:ablation_details}). V16b = bidirectional context encoder with VICReg (3 seeds). SIGReg = single shared encoder with spectral regularization, no EMA, no target copy (3 seeds).}
\label{tab:ablation}
\small
\begin{tabular}{@{}l|cc@{}}
\toprule
...
Bidirectional context (V16b) & 10.06$_{\pm1.4}$ & 15.06$_{\pm1.2}$ \\
SIGReg (no EMA) & 9.2$_{\pm1.5}$ & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Bidirectional context.} V16b (bidirectional context encoder with VICReg) achieves the best frozen probe (10.06 $\pm$ 1.4 RMSE, below supervised STAR 10.61), yet its label-efficiency crossover is inverted: at 5\% labels V16b E2E ($40.6 \pm 10.2$) is substantially worse than V2 E2E ($25.3 \pm 5.1$). Cross-domain transfer also degrades: FD001-pretrained V16b on FD003 yields 37.8 vs.\ 31.5 for V2. The causal inductive bias appears to confer a transferability advantage that bidirectional encoders do not share---likely because causal temporal ordering is a domain-invariant property.
