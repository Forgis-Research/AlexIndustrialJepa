...
\begin{enumerate}[nosep]
  ...
  \item Generalization to multivariate sensor streams (Appendix~\ref{app:turbofan}): a trajectory variant of JEPA applied to NASA C-MAPSS turbofan data achieves RMSE $= 14.23 \pm 0.39$ on FD001, outperforming the best published SSL result (AE-LSTM: 13.99) and beating a supervised LSTM at all five label budgets. Critically, we verify the result is not a population-mean artifact: the frozen encoder recovers the simulator's health index with $R^2 = 0.926$ without labels, and within-engine degradation correlation is $\rho = 0.830 \pm 0.023$ (5 seeds), with a $+41.5$ RMSE penalty when context vectors are shuffled across engines.
\end{enumerate}
...
\begin{enumerate}[nosep]
  \item \textbf{Beats prior SSL on C-MAPSS}: Traj JEPA E2E ($13.80_{\pm0.8}$, or $14.23 \pm 0.39$ across 5 seeds) outperforms AE-LSTM SSL (13.99), the only published SSL result on C-MAPSS FD001, without using failure-time labels during pretraining.
