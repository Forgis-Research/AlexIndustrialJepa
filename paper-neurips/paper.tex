...
\begin{table}[t]
\centering
\caption{Architecture ablations on C-MAPSS FD001 (3 seeds unless noted). Frozen = encoder locked; E2E = joint fine-tuning. ``w/o Full-sequence target'' is the future-only target variant (context encoder sees $\x_{1:t}$, target sees only $\x_{t+1:t+k}$); the full model uses full-sequence target for both context and target encoders. Bidirectional context = V16b (bidi encoder + VICReg regularization instead of EMA L1).}
\label{tab:ablation}
\small
\begin{tabular}{@{}l|cc@{}}
\toprule
& \multicolumn{2}{c}{FD001 RMSE $\downarrow$} \\
Model Variant & Frozen & E2E \\
\midrule
w/o Full-sequence target (future-only) & 17.81$_{\pm1.7}$ & --- \\
Bidirectional context (V16b) & 10.06$_{\pm1.4}$ & 15.06$_{\pm1.2}$ \\
SIGReg variant (no EMA, 3 seeds) & 9.2$_{\pm1.5}$ & --- \\
\midrule
\textbf{Full Model (causal + EMA, 5 seeds)} & \textbf{17.81}$_{\pm1.7}$ & \textbf{14.23}$_{\pm0.4}$ \\
\bottomrule
\end{tabular}
\end{table}
